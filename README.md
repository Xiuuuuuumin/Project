...
## WebSocket 使用指南（派車系統聊天）
### 1️⃣ Web 端
#### 1. 建立 WebSocket 連線
```
// 假設 order_id = 146f35ebdf7f47878f38351e40b1be0e
// sender_type = "user", sender_id = 19
const orderId = "146f35ebdf7f47878f38351e40b1be0e";
const senderType = "user";
const senderId = 19;

const ws = new WebSocket(
  `wss://8458c9a5736e.ngrok-free.app/ws/chat?order_id=${orderId}&sender_type=${senderType}&sender_id=${senderId}`
);
...
ws.onmessage = (event) => {
  ...
  if (data.type === "ping") {
    console.log("Heartbeat ping received:", data.timestamp);
  }
...
```
### 2️⃣ Flutter 端
使用 web_socket_channel 套件（可跨 Web / Android / iOS）
#### 1. 安裝套件
在 pubspec.yaml 中：
```
dependencies:
...
final orderId = "146f35ebdf7f47878f38351e40b1be0e";
final senderType = "user";
final senderId = 19;

final channel = WebSocketChannel.connect(
  Uri.parse('wss://8458c9a5736e.ngrok-free.app/ws/chat?order_id=$orderId&sender_type=$senderType&sender_id=$senderId'),
);
...
channel.stream.listen((message) {
  ...
  if (data['type'] == 'ping') {
    print("Heartbeat ping received: ${data['timestamp']}");
  }

  if (data['history'] != null) {
    print("Chat history: ${data['history']}");
  } else {
...
```
### 3️⃣ 心得 & 注意事項
1. WebSocket URL 的 query parameters
	- order_id：對應訂單
	- sender_type：user / driver
	- sender_id：整數，用戶或司機 ID
2. 心跳機制
	- Server 每 30 秒會送 ping，如果心跳失敗，會自動斷線並從房間移除 WebSocket
	- Client 不用特別回覆 ping，但可以用來確認連線是否正常
3. 歷史訊息
	- 新加入的連線會自動收到最近 50 條訊息
4. 錯誤處理
	- Client 建議加上 onerror / try-catch
